...
class WebDialogs:
    ...
    def get_value_from_user(self, message):
        requests.post(f"{self.server_url}/api/create/get_value_from_user", json={"message": message})
    ...
    def wait_for_user_selection(self, message, options):
        requests.post(
            f"{self.server_url}/api/create/get_selection_from_user", json={"message": message, "options": options}
        )
        return self.__get_response()
    ...
    def wait_for_user_selections(self, message, options):
        requests.post(
            f"{self.server_url}/api/create/get_selections_from_user", json={"message": message, "options": options}
        )
        return self.__get_response()
    ...
    def execute_manual_step(self, message, default_error=""):
        requests.post(f"{self.server_url}/api/create/execute_manual_step", json={"message": message})
    ...
    def pause_execution(self, message):
        requests.post(f"{self.server_url}/api/create/pause_execution", json={"message": message})
        return self.__get_response()
